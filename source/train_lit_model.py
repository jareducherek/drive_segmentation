...
def setup_callbacks_loggers(args):
    ...
    log_path = Path('/home/jared/Research/comma10k-baseline/LogFiles/')
    name = args.backbone
    version = args.version
    tb_logger = TensorBoardLogger(log_path, name=name, version=version)
    lr_logger = LearningRateMonitor(logging_interval='epoch')
    ckpt_callback = ModelCheckpoint(dirpath=Path(tb_logger.log_dir)/'checkpoints/', filename='{epoch:02d}_{val_loss:.4f}',
