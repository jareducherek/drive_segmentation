...
class LitModel(pl.LightningModule):
    ...
    def __build_model(self):
        ...
        self.net = smp.Trans2Seg(encoder_name=self.backbone,
                                 encoder_depth=4,
    ...
    def training_step(self, batch, batch_idx):
        ...
        for metric_name in metrics.keys():
            self.log(metric_name, metrics[metric_name], on_step=False, on_epoch=True, logger=True, prog_bar=True)
    ...
    def validation_epoch_end(self, outputs):
        ...
        for metric_name in keys:
            metrics[metric_name] = torch.cat([output[metric_name] for output in outputs]).mean()

        metrics['step'] = self.current_epoch

        for metric_name in metrics.keys():
            self.log(metric_name, metrics[metric_name])
